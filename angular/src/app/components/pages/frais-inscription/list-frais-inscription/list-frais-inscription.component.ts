...
@Component({
  selector: 'app-list-frais-inscription',
  templateUrl: './list-frais-inscription.component.html',
  styleUrls: ['./list-frais-inscription.component.scss']
})

export class ListFraisInscriptionComponent implements OnInit  {
  ...
  constructor(
    ...
    private fraisSercive : FraisInscriptionService,
  ...
  getFraisInscri(){
    this.fraisSercive.getFrais().subscribe(res=>{
      this.fraisList = res
    })  }
  ...
  goToEdit(data : any){
    ...
    console.log(data)
    const url = 'pages/frais-inscription/edit-frais-inscription/'+data.idRevenue
    this.router.navigateByUrl(url)
  }

  delete(item : any){
    console.log(item)
    if(confirm('did you want to delete this data ?')){
      this.fraisSercive.delete(item.idRevenue).subscribe(res=>{
        this.getFraisInscri()
        alert('OK')
      },err => {
        alert('Error server')
  ...
  edtatFianace(){
    this.http.put('http://localhost:8089/servicefinancier/getEtatFinance' , {}).subscribe(res=>{
      this.valeur = res
    })
  }
